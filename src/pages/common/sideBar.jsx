    ...
    return(
        <div className='flex flex-col h-full rounded-tr-md rounded-br-md bg-gradient-to-b from-sky-500 to-blue-700 column-1 w-1/4 bg-blue-500'>
          <ul className=' content-center ml-2  justify-items-start'>
             ...
             <SideBarElement linkto={'/404'} selected={selectedIndex === 2 ? true : false} icon={<UserIcon className=' h-6 w-6'/>} text={'Tutores'} />
             <SideBarElement linkto={'/404'} selected={selectedIndex === 3 ? true : false} icon={<AcademicCapIcon className=' h-6 w-6'/>} text={'Alumnos'} />             
          </ul>
        </div>
    )
}
